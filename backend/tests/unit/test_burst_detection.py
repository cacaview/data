...
def test_empty_dataframe_input():
    result = detect_burst_products(
        [{"hs_code": "x", "year": 2024, "month": 1, "trade_value_usd": 0}]
    )
...
def test_explosive_growth_triggers_burst():
    ...
    for month in range(1, 13):
        records.append(
            {"hs_code": "999999", "year": 2023, "month": month, "trade_value_usd": 1_000_000}
        )
    ...
    for month in range(1, 13):
        ...
        records.append(
            {"hs_code": "999999", "year": 2024, "month": month, "trade_value_usd": value}
        )
    result = detect_burst_products(records, threshold_pct=200.0)
